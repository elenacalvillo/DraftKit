...
RETURNS TABLE(
  ...
  content_last_edited_at timestamp with time zone,
...
AS $$
  ...
  base AS (
    ...
    UNION
    -- Project owner: chapters within a book they own (covers self-assigned book chapters
    -- even when the collaborator/requester link is missing).
  ...
  -- Collapse duplicates by request_id, preferring the strongest role
  ranked AS (
    SELECT request_id,
           role_in_workspace,
           joined_at,
