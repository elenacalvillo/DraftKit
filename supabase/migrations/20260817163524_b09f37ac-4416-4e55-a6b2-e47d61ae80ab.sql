DROP FUNCTION IF EXISTS public.add_project_member_by_creator(uuid, uuid, text);

CREATE OR REPLACE FUNCTION public.add_project_member_by_creator(_project_id uuid, _creator_id uuid, _role text)
 RETURNS TABLE(out_id uuid, out_project_id uuid, out_user_id uuid, out_email text, out_role text, out_invited_at timestamp with time zone, out_joined_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
